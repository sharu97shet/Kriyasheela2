...
function initializeVisia() {
	...
	// APPLY ONLINE FORM HIDE AND SHOW
	//CD BY MEGALA

	jQuery(".button").click(function () {
		jQuery("#OnlineForm").toggle();
	});
	...
	jQuery(function () {
		...
		BV.init();
		BV.show("video/visia-video.mp4", {
			altSource: "video/visia-video.ogv",
			ambient: true,
		});
	});
}
...
function initializePortfolio() {
	...
	jQuery(window).bind("hashchange", function () {
		...
		if (hash.substr(0, rootLength) != root) {
			...
			jQuery(".portfolio")
				.find(".projectlist.active-folio")
				.removeClass("active-folio");
			jQuery(".portfolio")
				.find(
					".ajax-content.active-ajax, .project-navigation.active-ajax, .closeProject.active-ajax, .loader.active-ajax"
				)
			...
			jQuery(".active-folio")
				...
				.children(".ajax-content, .project-navigation, .closeProject, .loader")
			...
			if (pageRefresh == true && hash.substr(0, rootLength) == root) {
				...
				jQuery("html,body")
					.stop()
					.animate({ scrollTop: scrollPostition + "px" }, 1000, function () {
						deleteProject();
					});
			}
		}
	...
	function deleteProject(closeURL) {
		...
		jQuery(".portfolio")
			.find(
				".ajax-content.active-ajax, .project-navigation.active-ajax, .closeProject.active-ajax"
			)
			.removeClass("active-ajax");
		portfolioGrid.find(".project.current").children().removeClass("active");
		portfolioGrid.find(".project.current").removeClass("current");
